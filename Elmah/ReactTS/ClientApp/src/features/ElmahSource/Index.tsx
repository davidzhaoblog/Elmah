...
import { getIndexVM, elmahSourceSelectors } from './Slice';
import { orderBys, ElmahSource } from './Types';
import Edit from 'src/components/ElmahSource/Edit';
import Search from 'src/components/ElmahSource/Search';
...
export default function IndexPage(): JSX.Element {
  ...
  const { commonCriteria, orderBy, queryPagingSetting } = store.getState().elmahSource;
  ...
  const openAdvancedSearchInPopup = (type: FormTypes, item: ElmahSource) => {
    setFormType(type);
    setOpenAdvancedSearchPopup(true);
    setSelectedItem(item);
  }

  const openFormInPopup = (type: FormTypes, item: ElmahSource) => {
    setFormType(type);
  ...
  const listItems = useSelector(
    (state: RootState) => elmahSourceSelectors.selectAll(state)
  );
