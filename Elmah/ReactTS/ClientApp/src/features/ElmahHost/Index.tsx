...
import { getIndexVM, elmahHostSelectors } from './Slice';
import { orderBys, ElmahHost } from './Types';
import Edit from 'src/components/ElmahHost/Edit';
import Search from 'src/components/ElmahHost/Search';
...
export default function IndexPage(): JSX.Element {
  ...
  const { commonCriteria, orderBy, queryPagingSetting } = store.getState().elmahHost;
  ...
  const openAdvancedSearchInPopup = (type: FormTypes, item: ElmahHost) => {
    setFormType(type);
    setOpenAdvancedSearchPopup(true);
    setSelectedItem(item);
  }

  const openFormInPopup = (type: FormTypes, item: ElmahHost) => {
    setFormType(type);
  ...
  const listItems = useSelector(
    (state: RootState) => elmahHostSelectors.selectAll(state)
  );
