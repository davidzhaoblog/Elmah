...
import { del } from 'src/features/ElmahType/Slice';
import { ElmahType } from 'src/features/ElmahType/Types';

function ListItem(props: IListItemProps<ElmahType>) {
  ...
  const { t } = useTranslation(["UIStringResource", "UIStringResourcePerApp"]);
    ...
    const handleDelete = () => {
        ...
        const deleteAlertDialog = {
            ...
            message: 'You are deleting ' + props.item.type,
    ...
    return (
        <Accordion key={props.item.type.toString()} expanded={expanded === 'panel1'} onChange={handleChange('panel1')}>
            ...
            <AccordionDetails>
                <div className={classes.column}>
                    ...
                    <Typography className={classes.heading} variant="h1" component="h1">{props.item.type}</Typography> class="hidden-sm"
                </div>
            </AccordionDetails>
            <Divider />
            <AccordionActions>
...
export default function List(props: IListProps<ElmahType>) {
    return (
        <div>
            {props.items.map((item: any) => {
                return (
                    <ListItem key={item.type} item={item} classes={props.classes} openFormInPopup={props.openFormInPopup} />
                );
            })}
        </div>
    );
