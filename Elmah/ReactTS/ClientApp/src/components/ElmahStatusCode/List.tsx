...
import { del } from 'src/features/ElmahStatusCode/Slice';
...
function ListItem(props: IListItemProps<ElmahStatusCode>) {
  ...
  const { t } = useTranslation(["UIStringResource", "UIStringResourcePerApp"]);
    ...
    return (
        <Accordion key={props.item.statusCode.toString()} expanded={expanded === 'panel1'} onChange={handleChange('panel1')}>
...
export default function List(props: IListProps<ElmahStatusCode>) {
    return (
        <div>
            {props.items.map((item: any) => {
                return (
                    <ListItem key={item.statusCode} item={item} classes={props.classes} openFormInPopup={props.openFormInPopup} />
                );
            })}
        </div>
    );
