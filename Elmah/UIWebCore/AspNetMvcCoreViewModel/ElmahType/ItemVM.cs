...
namespace Elmah.AspNetMvcCoreViewModel.ElmahType
{
    public partial class ItemVM : Elmah.ViewModelData.ElmahType.ItemVM
    {
        ...
        public virtual async Task Load(bool isToCompareType, string type
            , Framework.ViewModels.UIAction uiAction)
        {
            using (var scope = this.ServiceProvider.CreateScope())
            {
                try
                {
                    ...
                    var _Response = await thisService.GetMessageOfEntityByIdentifier(isToCompareType, type, -1, -1, null);
        ...
        public static async Task<ItemVM> CreateNewViewModel(IServiceProvider serviceProvider, Elmah.DataSourceEntities.ElmahType entity)
        {
            return await CreateNewViewModel<ItemVM>(serviceProvider, entity);
        }

        public static async Task<T> CreateNewViewModel<T>(IServiceProvider serviceProvider, Elmah.DataSourceEntities.ElmahType entity)
            where T: ItemVM, new()
        {
            var uiAction = Framework.ViewModels.UIAction.Create;
            T vm = new T();
            vm.ContentData.Title = string.Format("{0} {1}", Framework.Resx.UIStringResource.AddNew, Elmah.Resx.UIStringResourcePerApp.ElmahType);
            ...
            //vm.ContentData.Summary = string.Format("{0} {1}", Framework.Resx.UIStringResource.AddNew, Elmah.Resx.UIStringResourcePerApp.ElmahType);
