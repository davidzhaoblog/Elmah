...
namespace Elmah.AspNetMvcCoreViewModel.ElmahHost
{
    public partial class ItemVM : Elmah.ViewModelData.ElmahHost.ItemVM
    {
        ...
        public virtual async Task Load(bool isToCompareHost, string host
            , Framework.ViewModels.UIAction uiAction)
        {
            using (var scope = this.ServiceProvider.CreateScope())
            {
                try
                {
                    ...
                    var _Response = await thisService.GetMessageOfEntityByIdentifier(isToCompareHost, host, -1, -1, null);
        ...
        public static async Task<ItemVM> CreateNewViewModel(IServiceProvider serviceProvider, Elmah.DataSourceEntities.ElmahHost entity)
        {
            return await CreateNewViewModel<ItemVM>(serviceProvider, entity);
        }

        public static async Task<T> CreateNewViewModel<T>(IServiceProvider serviceProvider, Elmah.DataSourceEntities.ElmahHost entity)
            where T: ItemVM, new()
        {
            var uiAction = Framework.ViewModels.UIAction.Create;
            T vm = new T();
            vm.ContentData.Title = string.Format("{0} {1}", Framework.Resx.UIStringResource.AddNew, Elmah.Resx.UIStringResourcePerApp.ElmahHost);
            ...
            //vm.ContentData.Summary = string.Format("{0} {1}", Framework.Resx.UIStringResource.AddNew, Elmah.Resx.UIStringResourcePerApp.ElmahHost);
